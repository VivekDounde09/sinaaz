...
import { getChapterThreeData } from '@/utils/apiHandlers';
...
const Createkai = () => {
  ...
  const fetchChapterThreeData = async () => {
    setIsLoading(true);
    await getChapterThreeData().then((res) => {
      if (res?.status) {
        setIsLoading(false);
